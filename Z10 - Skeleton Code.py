...
class UndoState():
    ...
    def __init__(self, Targets, NumbersAllowed, Score):
        # Store all 3 attributes as a list of previous states of the game
        self.Targets = []
        self.NumbersAllowed = []
        self.Score = []
    
    def AddToUndo(self, NewTargets, NewNumbersAllowed, NewScore):
        self.Targets.append(NewTargets)
        self.NumbersAllowed.append(NewNumbersAllowed)
        self.Score.append(NewScore)
    
    def UndoLastTurn(self):
        # To undo, return the latest previous state of the game
        return self.Targets.pop(), self.NumbersAllowed.pop(), self.Score.pop()
...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    GameOver = False
    # Create the object
    CurrentState = UndoState(Targets, NumbersAllowed, Score)
    while not GameOver:
        DisplayState(Targets, NumbersAllowed, Score)
        # Per turn, add the current state of the game to the list of previous states
        CurrentState.AddToUndo(Targets, NumbersAllowed, Score)
        UserInput = input("Enter an expression: ")
        UndoInput = input(f'Undo last move?  You have {len(CurrentState.Targets)} undo(s) left. Enter Y/N: ').lower()
        if UndoInput == 'y':
            Targets, NumbersAllowed, Score = CurrentState.UndoLastTurn()
            # Continue to skip the rest of the code and re-iterate
            continue
        print()
