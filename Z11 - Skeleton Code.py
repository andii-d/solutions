...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    Score = 0
    GameOver = False
    IdentifiedTargets = [] # Initalise the list of identified targets
    while not GameOver:
        DisplayState(Targets, NumbersAllowed, Score, IdentifiedTargets)
        TargetSuggestions = input('Would you like up to 5 random suggestions including only 2-3 operands? Y/N: ').lower()
        if TargetSuggestions == 'y':
            IdentifiedTargets = list(GetRandomSuggestions(Targets, NumbersAllowed)) # Convert back to a list for simplicity's sake
            continue
...
# Bruteforce all possible solutions within 30 attemps for 5 random targets
def GetRandomSuggestions(Targets, NumbersAllowed):
    Suggestions = set()
    SuggestionsTested = 0
    
    while len(Suggestions) < 5:
        Temp = [num for num in NumbersAllowed]
    
        FirstOperand = random.choice(Temp)    
        Temp.remove(FirstOperand)
        SecondOperand = random.choice(Temp)
        Temp.remove(SecondOperand)

        # Randomise operators allowed
        ...
        # Multiplication
            ...
            if float(FirstOperand / SecondOperand) - Res == 0.0:
                if Res in Targets and Res != -1:
                    Suggestions.add(Res)

        ThirdOperand = random.choice(Temp) # Don't remove as there needs to be 3 minimum numbers
        Operator = random.randint(0,3)
        ...
        # Multiplication
            ...
            Res = math.floor(Res / ThirdOperand)
            if float(Res / ThirdOperand) - Res == 0.0:
                if Res in Targets and Res != -1:
                    Suggestions.add(Res)

        SuggestionsTested += 1
        if SuggestionsTested > 30:
            break
    return Suggestions
...
def DisplayState(Targets, NumbersAllowed, Score, IdentifiedTargets):
    DisplayTargets(Targets, IdentifiedTargets)
...
def DisplayTargets(Targets, IdentifiedTargets):
    print("|", end = '')
    for T in Targets:
        if T == -1:
            print(" ", end = '')
        else:
            if T in IdentifiedTargets:
                print(f'*{T}*', end = '')           
            else:
                print(T, end = '')
        print("|", end = '')
