...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    while not GameOver:
        ...
        if CheckIfUserInputValid(UserInput):
            ...
            # Store the variables of the invalid numbers and if there even are invalid numbers
            NumbersInExpressionValidity, InvalidNumbers = CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber)
            if NumbersInExpressionValidity and len(InvalidNumbers) < 1:
                IsTarget, Score, TargetsCleared, UserInputEvaluation = CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, Score)
                if IsTarget:
                    NumbersAllowed = RemoveNumbersUsed(UserInput, MaxNumber, NumbersAllowed)
                    NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber)
                    # Tell the user what was a target and how many times it was hit
                    print(f'{UserInputEvaluation} was a target and was cleared {TargetsCleared} time(s).')
            else:
                # State which invalid numbers were used no matter how large, even if outside the max number range
                print(f'You used invalid numbers: {InvalidNumbers}')
        Score -= 1
        if Targets[0] != -1:
            GameOver = True
        else:
...
def CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, Score):
    ...
    return UserInputEvaluationIsATarget, Score, TargetsCleared, UserInputEvaluation
...
def CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
    ...
    if len(InvalidNumbers) < 1: 
        return True, InvalidNumbers
    else:
        return False, InvalidNumbers
# END CHANGE
