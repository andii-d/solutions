    ...
    if UserInputEvaluation != -1:
        ...
        while Count < len(Targets):
            ...
            if Targets[Count] == UserInputEvaluation:
                # Add 2 to score
                Score += 2
                # Initialise however many targets there should be to remove
                TargetsToRemove = 0
                for i in range(Count+1, len(Targets)):
                    # Iterate from the next target to the end of the target list
                    TargetsToRemove += 1
                    # If another target has also been found
                    if Targets[i] == UserInputEvaluation:
                        # Increment the score by however many targets were needed to be removed
                        Score += TargetsToRemove * 2
                        # Remove the targets from the list
                        del Targets[Count - 1: Count + TargetsToRemove]
                        break
...
def UpdateTargets(Targets, TrainingGame, MaxTarget, MaxNumberOfTargets):
    ...
    Targets.pop()
    if TrainingGame:
        # Fill the targets up to the maximum number of targets for both training and standard game
