...
def Main():
    ...
    MaxNumber = 0
    TrainingGame = False
    Choice = input("Enter y to play the training game, L to load a previous game, anything else to play a random game: ").lower()
    print()
    if Choice == "y":
        MaxNumber = 1000
        MaxTarget = 1000
        ...
        while True:
            # Keep asking the user to input a valid game state until they have
            PreviousGame = input('Enter your previous game state: ')
            if len(PreviousGame) != 26:
                print('Please enter a valid game state as the inputted one has an incorrect length.')
                continue
            # Gather the required variables
            Targets, NumbersAllowed, Score = RestoreGame(PreviousGame)
            # Assume the game is not a training game thus insert parameters for MaxTarget and MaxNumber of such
            PlayGame(Targets, NumbersAllowed, False, 10, 50, Score)
            break
            # Break out of the loop otherwise the prompt to input a previous game state will continue to occur
    else:
        MaxNumber = 10
        MaxTarget = 50
...
# Initialise score to be 0 if there is no loaded game
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber, Score=0):
    GameOver = False
    while not GameOver:
...
def RestoreGame(PreviousGame):
    # Enter '@@@@@Qf_Hg@kCCN@WJhSCHGJAC' as the test case
    
    # Use list comprehension in order to convert characters to ASCII and accommodate for the empty spaces 
    PrevTargets = [(ord(char) - 64) if ord(char) != 64 else -1 for char in PreviousGame[:20]]
    PrevNumbersAllowed = [(ord(char) - 64) for char in PreviousGame[20:25]]
    PrevScore = ord(PreviousGame[-1]) - 64
    
    return PrevTargets, PrevNumbersAllowed, PrevScore
