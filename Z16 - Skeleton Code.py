...
def ConvertToRPNWithBrackets(UserInput, MaxNumber):
    ...
    RPNQueue = [] # Expression stack
    Precedence = {'(': 1, '+': 2, '-': 2, '*': 4, '/': 4}
    
    for char in UserInput:
        # Push numbers to stack first, whether in brackets or not 
        if CheckValidNumber(char, MaxNumber):
            RPNQueue.append(char)
        elif char == '(':
            # Push opening bracket to stack of operators
            Operators.append(char)
        elif char == ')':
            # Pop the rest of the operators within the expression containing brackets
            # Until the remaining operator is '('
            while Operators and Operators[-1] != '(':
                RPNQueue.append(Operators.pop())
            Operators.pop()  # Remove the '(' from stack at last
        else:
            # char is an operator +, -, *, /
    ...
    # Push any remaining operators to the expression stack
    while Operators:
        RPNQueue.append(Operators.pop())
...
def CheckIfUserInputValid(UserInput):
    # Add brackets to the allowed operators
