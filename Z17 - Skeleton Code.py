...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    while not GameOver:
        ...
        # Get the user's 
        IsPostfixInput = input('Enter y for postfix, n for infix notation: ').lower()
        if IsPostfixInput == 'y':
            IsPostFix = True
        else:
            IsPostFix = False
        # Tell the user to use , as a delimiter if they do want to enter an RPN expression
        UserInput = input(f"Enter an expression using , as a delimiter: " if IsPostFix else "Enter an expression: ")
        print()
        if CheckIfUserInputValid(UserInput, IsPostFix):
            UserInputInRPN = ConvertToRPN(UserInput, IsPostFix)
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
                IsTarget, Score = CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, Score)
                if IsTarget:
                    NumbersAllowed = RemoveNumbersUsed(UserInput, MaxNumber, NumbersAllowed, IsPostFix)
                    NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber)
        Score -= 1
        if Targets[0] != -1:
            GameOver = True
...
def RemoveNumbersUsed(UserInput, MaxNumber, NumbersAllowed, IsPostFix):
    # To remove the numbers used from the expression entered, again input the state of IsPostfix
    UserInputInRPN = ConvertToRPN(UserInput, IsPostFix)
...
# Assume by default that IsPostfix is False, but if it is true, just return the input split by the delimiter. 
def ConvertToRPN(UserInput, IsPostfix=False):
    if UserInput and IsPostfix:
...
def CheckIfUserInputValid(UserInput, IsPostFix):
    # Assume the user's input is valid 
    if UserInput and IsPostFix:
        return True
