...
def CreateTargets(SizeOfTargets, MaxTarget):
    ...
    # Continue to generate targets until there are no duplicates at all
    while len(Targets) < SizeOfTargets:
        Number = GetNumber(MaxTarget)
...
def FillNumbers(NumbersAllowed, TrainingGame, MaxNumber):
    if TrainingGame:
        ...
        # Continue to generate numbers allowed (new or current) with no duplicates at all
        UniqueNumbersAllowed = set(NumbersAllowed)
        while len(UniqueNumbersAllowed) < 5:
            Number = GetNumber(MaxNumber)
            if Number in UniqueNumbersAllowed:
                continue
            else:
                UniqueNumbersAllowed.add(Number)
        return list(UniqueNumbersAllowed)
